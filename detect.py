...
def drawPred(classId, conf, left, top, right, bottom):
    ...
    lp = frame[top:bottom, left:right]
    if lp.shape[0] > 0 and lp.shape[1] > 0:
        gray = cv.cvtColor(lp, cv.COLOR_BGR2RGB)

        results = text_reader.readtext(lp, paragraph = True)

        d = datetime.date.today() 
        currentDateAndTime = datetime.datetime.now()
        t = currentDateAndTime.strftime("%H:%M:%S")

        for (bbox, text) in results:
            if any(d for substring in date):
                date.append(d)
                time.append(t)
                no.append(text)
